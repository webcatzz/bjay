...
func _ready() -> void:
	position -= Vector2(Map.size + Vector2i(1, -1)) * SEPARATION * 0.5
	for place: Place in Map.places():
		...
		if place.coords.x < Game.place.coords.x or place.coords.x == Game.place.coords.x and place.coords.y != Game.place.coords.y or not place.type.icon:
			node = preload("res://scenes/route/phases/night/map_star.tscn").instantiate()
		else:
			node = Sprite2D.new()
...
func _draw() -> void:
	for place: Place in Map.places():
		...
		var method: Callable = draw_dashed_line if place.coords.x == Map.size.x - 1 else draw_line
